...
NCCL<Dtype>::NCCL(shared_ptr<Solver<Dtype> > solver, const string& uid)
  ...
  LOG(INFO)<<"Configuring Solver for NCCL...";
  this->Configure(solver.get());
  LOG(INFO)<<"Setting multiple process for NCCL...";
  Caffe::set_multiprocess(true);
  ncclUniqueId nccl_uid;
  LOG(INFO)<<"Mem-copy for NCCL...";
  memcpy(&nccl_uid, &uid[0], NCCL_UNIQUE_ID_BYTES);  // NOLINT(caffe/alt_fn)
  LOG(INFO)<<"NCCL commit...";
  LOG(INFO)<<Caffe::solver_count();
  LOG(INFO)<<Caffe::solver_rank();
  LOG(INFO)<<"NCCL commit...";
